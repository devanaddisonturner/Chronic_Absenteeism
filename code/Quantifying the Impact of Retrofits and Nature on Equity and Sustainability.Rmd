...
p <- ggplot(CASchools_YearBuilt_Data, aes(x = Year_built))
  p <- p + geom_line(aes(y = Land_Surface_Temp, color = "Land Surface Temperature"))
  # adding the NDVI, transformed to match roughly the range of the temperature
  p <- p + geom_line(aes(y = NDVI_median*40, color = "Normalized Difference Vegetation Index (NDVI)"))
  # now adding the secondary axis, following the example in the help file ?scale_y_continuous
  # and, very important, reverting the above transformation
  p <- p + scale_y_continuous(sec.axis = sec, name = "Average Annual NDVI") 
  # modifying colors and theme options
  p <- p + scale_colour_manual(values = c("red", "blue"))
  p <- p + labs(y = "Average Annual Temperature [°C]",
                x = "Year Built of K-12 Schools",
                color = "Parameter") +
  ggtitle("K-12 Schools Infrastructure, Environment, and Climate Across California, USA during the COVID-19 Pandemic") + theme_bw() + theme(plot.title = element_text(hjust = 0.5))  # Center ggplot title
