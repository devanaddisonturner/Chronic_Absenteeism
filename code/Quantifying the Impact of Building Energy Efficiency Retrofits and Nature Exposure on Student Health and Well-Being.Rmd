...
ChronicAbsenteeism_FinancialCost %>%
  ...
  ggplot(., 
            aes(x = Intervention, 
                y = Potential_Financial_Cost_Savings, 
                # color = Intervention, 
                fill = Potential_Financial_Cost_Savings < 0
                )) +
  geom_bar(stat = "identity", position = "identity", show.legend = F) +
  geom_text(aes(label = round(Potential_Financial_Cost_Savings, digits = 0)), vjust = -0.3) + 
...
ggsave(filename = './figures/ChronicAbsenteeism_InterventionCost.png', plot = last_plot(), 
