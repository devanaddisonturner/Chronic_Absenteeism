...
# Multi-Physics Simulation Graph
...
# --- 3. Set Optimal Viewing Angle for Static Image ---
# Adjust theta (horizontal rotation) and phi (vertical tilt) for best perspective
# theta = 45: Shows both X and Y axes well
# phi = 25: Slight elevation to see the regression plane slope clearly
# fov = 30: Narrower field of view reduces distortion for publication-quality images

rgl.viewpoint(theta = 45, phi = 25, fov = 30, zoom = 0.85)

# Alternative angles to try (uncomment to test):
# rgl.viewpoint(theta = 60, phi = 30, fov = 30, zoom = 0.85)  # More emphasis on Y-axis
# rgl.viewpoint(theta = 30, phi = 20, fov = 30, zoom = 0.85)  # More emphasis on X-axis
# rgl.viewpoint(theta = 45, phi = 35, fov = 30, zoom = 0.80)  # Higher elevation view

# --- 4. Save as High-Resolution PNG (300 DPI) ---
# For publication quality at 300 DPI:
# - 6 inches wide = 1800 pixels
# - 5 inches tall = 1500 pixels

output_file <- "Absenteeism_Causal_Effect_3D.png"

# rgl.snapshot() captures the current view as a PNG
# For higher resolution, use snapshot3d() with explicit dimensions
snapshot3d(
  filename = output_file,
  width = 1800,    # 6 inches at 300 DPI
  height = 1500,   # 5 inches at 300 DPI
  webshot = FALSE  # Use rgl's native rendering
)

# Print confirmation
cat("Static image saved as:", file.path(getwd(), output_file), "\n")
cat("Resolution: 1800 x 1500 pixels (6\" x 5\" at 300 DPI)\n")
