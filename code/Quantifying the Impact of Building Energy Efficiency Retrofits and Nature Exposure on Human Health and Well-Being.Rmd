...
CASchools_PSM_Datacomplete <- complete(CASchools_PM_Dataimp)
...
#Number of rows that would not be removed due to missing data
nrow(na.omit(CASchools_PSM_Datacomplete))
...
maxmin_scalar_sub <- function(x){
  (x- min(x, na.rm = T)) / (max(x, na.rm = T) - min(x, na.rm = T))
  }
scaling_maxmin_L_complete <- as.data.frame(lapply(CASchools_PSM_Data_subL2, maxmin_scalar))
...
scaling_maxmin_H_complete <- as.data.frame(lapply(CASchools_PSM_Data_subH2, maxmin_scalar))
...
scaling_maxmin_B_complete <- as.data.frame(lapply(CASchools_PSM_Data_subB2, maxmin_scalar))
...
scaling_maxmin_P_complete <- as.data.frame(lapply(CASchools_PSM_Data_subP2, normalize))
