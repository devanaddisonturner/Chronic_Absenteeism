...
ggplot(Schools_Environment_Data, aes(x = NDVI_Under_Over_Avg, y = NDVI_median, color = Land_Surface_Temp)) +
  geom_quasirandom() +
 # scale_fill_manual() +
  labs(title = "Environmental Health of K-12 Schools in California, USA",
       x = "Environmental Health",
       y = "Median Normalized Difference Vegetation Index",
      fill = "Land Surface Temperature (°C)") + 
  theme_bw() + theme(plot.title = element_text(hjust = 0.5))  # Center ggplot title
ggsave(filename = './figures/', 'NDVI_LST_Schools.png', plot = last_plot(), width = 9, height = 7, units = "in", dpi = 300) 
```
...
library(ggplot2)
ggplot() + 
  geom_line(data=Treatment_Count_Data_All_coord, aes(x=Percent_Water, y=Land_Surface_Temp, color="Water"), size=2) + 
  geom_line(data=Treatment_Count_Data_All_coord, aes(x=Percent_Pasture, y=Land_Surface_Temp, color="Pasture"), size=2) + 
  geom_line(data=Treatment_Count_Data_All_coord, aes(x=Percent_Forest, y=Land_Surface_Temp, color="Forest"), size=2) +
  geom_line(data=Treatment_Count_Data_All_coord, aes(x=Percent_Developed, y=Land_Surface_Temp, color="Developed"), size=2)+ theme_bw() +
...
ggsave(filename = './figures/', 'LULC_NDVI_LST.png', plot = last_plot(), width = 9, height = 7, units = "in", dpi = 300) 
```
