...
## Scenario 1: Lighting Retrofits in K-12 Schools 
d1L_imp <- cohen.d(m_data_wide_2_complete_L$Chronic_Absenteeism_raw.x, m_data_wide_2_complete_L$Chronic_Absenteeism_raw.y)
d1L_imp
es1L_imp <- d1L_imp$estimate
...
## Graph Effect Size
### Scenario 1L: Lighting Retrofits 
```{r}
...
#' Here we use the effect size in scenario 1,Impact of
#'  Energy Efficiency Retrofits 
ES <- round(es1L_imp, digits = 2) 


#' get mean2 depending on value of ES from 
#'  d = (u1 - u2)/sd
#'  es = (mean1 - mean2)/sd
mean_c <- mean(m_data_wide_2_complete_L$Chronic_Absenteeism.x, na.rm = T)
mean2 = mean_c 
...
ggplot(df1, aes(x,y, color="control")) + 
  ...
  geom_vline(xintercept = mean1, linetype="dotted", color = 'red') +  
  ggtitle(paste0(
    "Scenario 1: Impact of Lighting Retrofits on Chronic Absenteeism in K-12 Schools",
    "\nCohen's d = ",  ES, ' (Neglible Effect)')) +
  # # change colors and legend annotation
  scale_color_manual("Group",
            values= c("treatment" = "red","control" = "black")) + 
...
ggsave(filename = 'effect_size_scenario1L.png', plot = last_plot(), width = 7, height = 4, units = "in", dpi = 300)
```
...
f <- paste0('./figures/', 'treatment_count_byScenario_byRetrofitType2.png')
ggsave(filename = f, plot = last_plot(), width = 10, height = 7, units = "in", dpi = 300)
```
...
power.analysis <- pwr.t.test(n = NULL, d = -0.11, sig.level = 0.05, power = 0.8, alternative = "two.sided") 
plot(power.analysis) +  
  theme_bw() + 
...
## Scenario 1L: Lighting Retrofits on Chronic absenteeism
power.analysis <- pwr.t.test(n = NULL, d = -0.28, sig.level = 0.05, power = 0.8, alternative = "two.sided") 
plot(power.analysis) +  
  theme_bw() + 
  labs(title = "Statistical Power - Scenario 1:", 
       subtitle = "The Impact of Lighting Retrofits on Chronic Absenteeism in K-12 Schools",
       x = "Sample Size", y = "P-value", 
       color = "Effect Size") + 
  theme(
    plot.title = element_text(size=8, face = 'bold'), 
    plot.subtitle =  element_text(size=8))  # Center ggplot title
ggsave(filename = paste0('./figures/', 'PowerAnalysis_LightingRetrofits.png'), plot = last_plot(), width = w, height = h, units = "in", dpi = 300
...
power.analysis <- pwr.t.test(n = NULL, d = 0.37, sig.level = 0.05, power = 0.8, alternative = "two.sided") 
plot(power.analysis) +  theme_bw() + 
labs(title = "Statistical Power - Scenario 3: ",
...
# Spherical Global Map [YJL: not sure what's this - may not needed for your publication]
```{r}
...
# Interactive Map [YJL: not sure how to do this - may not needed for your publication]
```{r}
...
ggsave(filename = paste0('./figures/', 'Year_Built_Schools.png'), plot = last_plot(), width = 6, height = 4, units = "in", dpi = 300)
```
