...
CASchools_PSM_Data <- read_csv("CA_Schools_PSM_Data_3.csv")



#CASchools_PSM_Data  <- read_xlsx("CA_Schools_PSM_Data_3.xlsx")


### Subset Data

CASchools_PSM_Data_subL <- subset(CASchools_PSM_Data, EE_Retrofit_Type == "Lighting")
CASchools_PSM_Data_subH <- subset(CASchools_PSM_Data, EE_Retrofit_Type == "HVAC")
CASchools_PSM_Data_subB <- subset(CASchools_PSM_Data, EE_Retrofit_Type == "Building_Envelope")
CASchools_PSM_Data_subP <- subset(CASchools_PSM_Data, EE_Retrofit_Type == "Pumps_Motors_Drives")
...
CASchools_PSM_Data2 <- CASchools_PSM_Data %>%
  dplyr::mutate(
    ...
    Students_with_Disabilities = as.numeric(Students_Disabilities),
...
#png(filename = f, width = 7, height = 5, units = "in", res = 300)
#cex.axis.size = 0.425
aggr(CASchools_PSM_Data_subL2, cex.axis=0.425)
#dev.off()

#Number of rows that would not be removed due to missing data
nrow(na.omit(CASchools_PSM_Data_subL2))

# Multivariate Imputation by Chained-Equations (MICE)
#fill missing values in dataset by prediction through multiple iterations using
#the MICE algorithm
set.seed(27)
CASchools_subL2imp <- mice(CASchools_PSM_Data_subL2, seed = 27)

# Complete dataset after imputation and store as a new dataframe
CASchools_subL2complete <- complete(CASchools_subL2imp)

#Visualize Proportion of missing data in Imputed Dataset
#f <- paste0('./figures/', 'data_missing_Imputed.png'); print(f)
##png(filename = f, width = 7, height = 5, units = "in", res = 300)
aggr(CASchools_subL2complete, cex.axis= 0.425)
#dev.off()

#Number of rows that would not be removed due to missing data
nrow(na.omit(CASchools_subL2complete))
...
#png(filename = f, width = 7, height = 5, units = "in", res = 300)
#cex.axis.size = 0.425
aggr(CASchools_PSM_Data_subH2, cex.axis=0.425)
#dev.off()

#Number of rows that would not be removed due to missing data
nrow(na.omit(CASchools_PSM_Data_subH2))

# Multivariate Imputation by Chained-Equations (MICE)
#fill missing values in dataset by prediction through multiple iterations using
#the MICE algorithm
set.seed(27)
CASchools_subH2imp <- mice(CASchools_PSM_Data_subH2, seed = 27)

# Complete dataset after imputation and store as a new dataframe
CASchools_subH2complete <- complete(CASchools_subH2imp)

#Visualize Proportion of missing data in Imputed Dataset
#f <- paste0('./figures/', 'data_missing_Imputed.png'); print(f)
##png(filename = f, width = 7, height = 5, units = "in", res = 300)
aggr(CASchools_subH2complete, cex.axis= 0.425)
#dev.off()

#Number of rows that would not be removed due to missing data
nrow(na.omit(CASchools_subH2complete))
...
#png(filename = f, width = 7, height = 5, units = "in", res = 300)
#cex.axis.size = 0.425
aggr(CASchools_PSM_Data_subB2, cex.axis=0.425)
#dev.off()

#Number of rows that would not be removed due to missing data
nrow(na.omit(CASchools_PSM_Data_subB2))

# Multivariate Imputation by Chained-Equations (MICE)
#fill missing values in dataset by prediction through multiple iterations using
#the MICE algorithm
set.seed(27)
CASchools_subB2imp <- mice(CASchools_PSM_Data_subB2, seed = 27)

# Complete dataset after imputation and store as a new dataframe
CASchools_subB2complete <- complete(CASchools_subB2imp)

#Visualize Proportion of missing data in Imputed Dataset
#f <- paste0('./figures/', 'data_missing_Imputed.png'); print(f)
##png(filename = f, width = 7, height = 5, units = "in", res = 300)
aggr(CASchools_subB2complete, cex.axis= 0.425)
```

```{r EE Retrofit --- 4. Pumps}
library(lattice)
library(VIM)
library(mice)
...
#png(filename = f, width = 7, height = 5, units = "in", res = 300)
#cex.axis.size = 0.425
aggr(CASchools_PSM_Data_subP2, cex.axis=0.425)
#dev.off()

#Number of rows that would not be removed due to missing data
nrow(na.omit(CASchools_PSM_Data_subP2))

# Multivariate Imputation by Chained-Equations (MICE)
#fill missing values in dataset by prediction through multiple iterations using
#the MICE algorithm
set.seed(27)
CASchools_subP2imp <- mice(CASchools_PSM_Data_subP2, seed = 27)

# Complete dataset after imputation and store as a new dataframe
CASchools_subP2complete <- complete(CASchools_subP2imp)


#Visualize Proportion of missing data in Imputed Dataset
#f <- paste0('./figures/', 'data_missing_Imputed.png'); print(f)
##png(filename = f, width = 7, height = 5, units = "in", res = 300)
aggr(CASchools_subP2complete, cex.axis= 0.425)
#dev.off()
```
...
scaling_maxmin_complete <- as.data.frame(lapply(CASchools_PSM_Datacomplete, maxmin_scalar))

scaling_maxmin_complete$School_ID <- rownames(CASchools_PSM_Datacomplete)
```
...
scaling_maxmin_L <- as.data.frame(lapply(CASchools_PSM_Data_subL2, maxmin_scalar))

scaling_maxmin_L$School_ID <- rownames(CASchools_PSM_Data_subL2)

scaling_maxmin_L_dropna <- scaling_maxmin_L %>%
  tidyr::drop_na()
...
scaling_maxmin_L_complete <- as.data.frame(lapply(CASchools_subL2complete, maxmin_scalar))

scaling_maxmin_L_complete$School_ID <- rownames(CASchools_subL2complete)
```
...
scaling_maxmin_H <- as.data.frame(lapply(CASchools_PSM_Data_subH2, maxmin_scalar))

scaling_maxmin_H$School_ID <- rownames(CASchools_PSM_Data_subH2)

scaling_maxmin_H_dropna <- scaling_maxmin_H %>%
  tidyr::drop_na()
...
scaling_maxmin_H_complete <- as.data.frame(lapply(CASchools_subH2complete, maxmin_scalar))

scaling_maxmin_H_complete$School_ID <- rownames(CASchools_subH2complete)
```
...
scaling_maxmin_B <- as.data.frame(lapply(CASchools_PSM_Data_subB2, maxmin_scalar))

scaling_maxmin_B$School_ID <- rownames(CASchools_PSM_Data_subB2)

scaling_maxmin_B_dropna <- scaling_maxmin_B %>%
  tidyr::drop_na()
...
scaling_maxmin_B_complete <- as.data.frame(lapply(CASchools_subB2complete, maxmin_scalar))

scaling_maxmin_B_complete$School_ID <- rownames(CASchools_subB2complete)  
...
scaling_maxmin_P <- as.data.frame(lapply(CASchools_PSM_Data_subP2, maxmin_scalar))

scaling_maxmin_P$School_ID <- rownames(CASchools_PSM_Data_subP2)

scaling_maxmin_P_dropna <- scaling_maxmin_P %>%
  tidyr::drop_na()
...
scaling_maxmin_P_complete_3 <- as.data.frame(lapply(CASchools_subP2complete, maxmin_scalar))

scaling_maxmin_P_complete_3$School_ID <- rownames(CASchools_subP2complete)  
```
...
#### Scenario 1B: Building Enevelope Retrofits in K-12 Schools with Imputed Dataset
...
m_data3_complete <- match.data(m.out3_complete_B)
```
...
## left join raw outcome data
raw_complete <- CASchools_PSM_Datacomplete %>%
  dplyr::select(Chronic_Absenteeism) %>%
  dplyr::mutate(School_ID = rownames(.)) %>%
  dplyr::rename('Chronic_Absenteeism_raw' = 'Chronic_Absenteeism')
...
## left join raw outcome data
raw_complete_L <- CASchools_subL2complete %>%
  dplyr::select(Chronic_Absenteeism) %>%
  dplyr::mutate(School_ID = rownames(.)) %>%
  dplyr::rename('Chronic_Absenteeism_raw' = 'Chronic_Absenteeism')
...
## left join raw outcome data
raw_complete_H <- CASchools_subH2complete %>%
  dplyr::select(Chronic_Absenteeism) %>%
  dplyr::mutate(School_ID = rownames(.)) %>%
  dplyr::rename('Chronic_Absenteeism_raw' = 'Chronic_Absenteeism')
...
## left join raw outcome data
raw_2_complete <- CASchools_PSM_Datacomplete %>%
  dplyr::select(Chronic_Absenteeism) %>%
  dplyr::mutate(School_ID = rownames(.)) %>%
  dplyr::rename('Chronic_Absenteeism_raw' = 'Chronic_Absenteeism')
...
## left join raw outcome data
raw_3_complete <- CASchools_PSM_Datacomplete %>%
  dplyr::select(Chronic_Absenteeism) %>%
  dplyr::mutate(School_ID = rownames(.)) %>%
  dplyr::rename('Chronic_Absenteeism_raw' = 'Chronic_Absenteeism')
...
## left join raw outcome data
raw_3_complete_L <- CASchools_subL2complete %>%
  dplyr::select(Chronic_Absenteeism) %>%
  dplyr::mutate(School_ID = rownames(.)) %>%
  dplyr::rename('Chronic_Absenteeism_raw' = 'Chronic_Absenteeism')
...
## left join raw outcome data
raw_3_complete_H <- CASchools_subH2complete %>%
  dplyr::select(Chronic_Absenteeism) %>%
  dplyr::mutate(School_ID = rownames(.)) %>%
  dplyr::rename('Chronic_Absenteeism_raw' = 'Chronic_Absenteeism')
...
```{r Scenario 3B}
### Scenario 3B: Building Envelope Retrofits in K-12 Schools with Imputed Data
#Control Group 
m_data_c_complete_B <- m_data_complete_B %>%
  dplyr::filter(retrofit == 0)

#Treatment Group
m_data_t_complete_B <- m_data_complete_B %>%
  dplyr::filter(retrofit == 1)

## left join raw outcome data
raw_complete_B <- CASchools_subB2complete %>%
  dplyr::select(Chronic_Absenteeism) %>%
  dplyr::mutate(School_ID = rownames(.)) %>%
  dplyr::rename('Chronic_Absenteeism_raw' = 'Chronic_Absenteeism')
...
```{r Scenario 3P}
### Scenario 3P: Pumps, Motors, & Drives Retrofits in K-12 Schools with Imputed Data
#Control Group 
m_data_c_complete_P <- m_data_complete %>%
  dplyr::filter(retrofit == 0)

#Treatment Group
m_data_t_complete_P <- m_data_complete %>%
  dplyr::filter(retrofit == 1)

## left join raw outcome data
raw_complete_P <- CASchools_subP2complete %>%
  dplyr::select(Chronic_Absenteeism) %>%
  dplyr::mutate(School_ID = rownames(.)) %>%
  dplyr::rename('Chronic_Absenteeism_raw' = 'Chronic_Absenteeism')
...
m_data_t_withRaw_complete_P <- m_data_t_complete %>%
  left_join(., raw_complete_P, by = 'School_ID')
...
## Scenario 1B: Pumps, Motors, and Drives Retrofits in K-12 Schools with Imputed Data
TTestS1_complete_B <- t.test(m_data_wide_2_complete_B$Chronic_Absenteeism_raw.x, m_data_wide_2_complete_B$Chronic_Absenteeism_raw.y, paired = TRUE, conf.level = 0.95)
print(TTestS1_complete_B)
