...
## Scenario 3L: Lighting Retrofits and Nature Exposure in K-12 Schools 
d3L_imp <- cohen.d(m_data_wide_6_complete_L$Chronic_Absenteeism_raw.x, m_data_wide_6_complete_L$Chronic_Absenteeism_raw.y)
d3L_imp
es3L_imp <- d3L_imp$estimate

## Scenario 3H: HVAC Retrofits and Nature Exposure in K-12 Schools 
...
#Method 1: Read individual data file to obtain the total counts of treatments across each scenario and interventions 
#including subset of EE retrofit
#for the following variables (i) retrofit, (ii) ndvi_binary, (iii) retrofit and ndvi
...
Treatment_Count_Data_All <- read_csv("./data/Treatment_Count_Data_All_coord.csv")
...
library(dplyr)
#calculate total number of interventions in each scenario by unique K-12 school
Treatment_Count_Data_All_2 <- Treatment_Count_Data_All %>%
                              group_by(CDSCode) %>%
                              summarise(Intervention_Total_Count_S1 = sum(retrofit), Intervention_Total_Count_S2 = sum(ndvi_median_binary), Intervention_Total_Count_S3 = sum(retrofit_and_ndvi))
sum(Treatment_Count_Data_All_2[, 'Intervention_Total_Count_S1'], na.rm = TRUE)
sum(Treatment_Count_Data_All_2[, 'Intervention_Total_Count_S2'], na.rm = TRUE)
sum(Treatment_Count_Data_All_2[, 'Intervention_Total_Count_S3'], na.rm = TRUE)


## Subset Data by 4 types of Energy Efficiency Retrofits

#1. Lighting 
Treatment_Count_Data_All_subL <- subset(Treatment_Count_Data_All, EE_Retrofit_Type == "Lighting")
library(dplyr)
#calculate total number of interventions in each scenario by unique K-12 school
Treatment_Count_Data_All_subL_2 <- Treatment_Count_Data_All_subL %>%
                              group_by(CDSCode) %>%
                              summarise(Intervention_Total_Count_S1 = sum(retrofit), Intervention_Total_Count_S3 = sum(retrofit_and_ndvi))
sum(Treatment_Count_Data_All_subL_2[, 'Intervention_Total_Count_S1'], na.rm = TRUE)
sum(Treatment_Count_Data_All_subL_2[, 'Intervention_Total_Count_S3'], na.rm = TRUE)

#2. HVAC
Treatment_Count_Data_All_subH <- subset(Treatment_Count_Data_All, EE_Retrofit_Type == "HVAC")
library(dplyr)
#calculate total number of interventions in each scenario by unique K-12 school
Treatment_Count_Data_All_subH_2 <- Treatment_Count_Data_All_subH %>%
                              group_by(CDSCode) %>%
                              summarise(Intervention_Total_Count_S1 = sum(retrofit), Intervention_Total_Count_S3 = sum(retrofit_and_ndvi))
sum(Treatment_Count_Data_All_subH_2[, 'Intervention_Total_Count_S1'], na.rm = TRUE)
sum(Treatment_Count_Data_All_subH_2[, 'Intervention_Total_Count_S3'], na.rm = TRUE)

#3. Building Envelope
Treatment_Count_Data_All_subB <- subset(Treatment_Count_Data_All, EE_Retrofit_Type == "Building_Envelope")
library(dplyr)
#calculate total number of interventions in each scenario by unique K-12 school
Treatment_Count_Data_All_subB_2 <- Treatment_Count_Data_All_subB %>%
                              group_by(CDSCode) %>%
                              summarise(Intervention_Total_Count_S1 = sum(retrofit), Intervention_Total_Count_S3 = sum(retrofit_and_ndvi))
sum(Treatment_Count_Data_All_subB_2[, 'Intervention_Total_Count_S1'], na.rm = TRUE)
sum(Treatment_Count_Data_All_subB_2[, 'Intervention_Total_Count_S3'], na.rm = TRUE)


#4. Pumps, Motors, & Drives
Treatment_Count_Data_All_subP <- subset(Treatment_Count_Data_All, EE_Retrofit_Type == "Pumps_Motors_Drives")
library(dplyr)
#calculate total number of interventions in each scenario by unique K-12 school
Treatment_Count_Data_All_subP_2 <- Treatment_Count_Data_All_subP %>%
                              group_by(CDSCode) %>%
                              summarise(Intervention_Total_Count_S1 = sum(retrofit), Intervention_Total_Count_S3 = sum(retrofit_and_ndvi))
sum(Treatment_Count_Data_All_subP_2[, 'Intervention_Total_Count_S1'], na.rm = TRUE)
sum(Treatment_Count_Data_All_subP_2[, 'Intervention_Total_Count_S3'], na.rm = TRUE)
```

```{r}
# Number of Optimal Pairs-Matched for K-12 Schools in each policy intervention/scenario 
...
### Scenario 1L: Lighting Retrofits in K-12 Schools
length(unique(m_data_wide_2_complete_L$subclass))

### Scenario 1H: HVAC Retrofits in K-12 Schools
length(unique(m_data_wide_2_complete_H$subclass))

### Scenario 1B: Building Envelope Retrofits in K-12 Schools
length(unique(m_data_wide_2_complete_B$subclass))

### Scenario 1P: Pumps, Motors, & Drives in K-12 Schools
...
### Scenario 3B: Building Envelope Retrofits and Nature Exposure in K-12 Schools
length(unique(m_data_wide_6_complete_B$subclass))

### Scenario 3P: Pumps/Motors/&Drives Retrofits and Nature Exposure in K-12 Schools
...
ggsave(filename = './figures/', 'Prop39_Schools_Global_Map.png', plot = last_plot(), width = 6, height = 4, units = "in", dpi = 300)
```


# Spherical Global Map
```{r}
# Spherical World Map with our Data 
library(sf)
df <-  sf::sf_project("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0", 
                      "+proj=moll", Treatment_Count_Data_All_coord) %>%
       as.data.frame() %>%
...
world_map_2 <- world_map %>% 
  ...
  geom_text(data = df, aes(LONGITUDE, LATITUDE, label = object),
...
Prop_39_schools_map <- mapview(Treatment_Count_Data_All_coord, 
                               ...
                               grid = FALSE)
Prop_39_schools_map
# save with OpenTopoMap baackground layer
library(webshot)
mapshot(Prop_39_schools_map, delay = 10, 
...
library(dplyr)
library(readr)
Schools_Year_Built <- read_csv("Schools_Year_Built_Analysis_Data.csv")
...
ggsave(filename = './figures/', 'Year_Built_Schools.png', plot = last_plot(), width = 6, height = 4, units = "in", dpi = 300)
```
## Environment
```{r}
...
library(readr)
library(ggplot2)
Schools_Environment_Data <- read_csv("Schools_Environment_Data.csv")
# Load the required libraries
install.packages("ggbeeswarm")
library(ggbeeswarm)
library(ggplot2)
library(viridis)
ggplot(Schools_Environment_Data, aes(x = NDVI_Under_Over_Avg, y = NDVI_median, color = Land_Surface_Temp)) +
  geom_quasirandom() + 
 # scale_fill_manual(low = "blue", high = "pink") +
 # scale_fill_manual() +
  labs(title = "Relationship of NDVI and LST near K-12 Schools in California, USA",
       x = "Environmental Health",
       y = "Median Normalized Difference Vegetation Index") + 
      labs(color = "(°C)") +
  theme_bw() + theme(plot.title = element_text(hjust = 0.5))  # Center ggplot title
ggsave(filename = 'NDVI_LST_Schools.png', plot = last_plot(), width = 9, height = 7, units = "in", dpi = 300)
```

```{r}
#Graph the Relationship of LULC, NDVI, and LST
library(ggplot2)
ggplot() + 
  geom_point(data=Treatment_Count_Data_All_coord, aes(x=Percent_Water, y=Land_Surface_Temp, color="Water"), size=2) + 
  geom_point(data=Treatment_Count_Data_All_coord, aes(x=Percent_Pasture, y=Land_Surface_Temp, color="Pasture"), size=2) + 
  geom_point(data=Treatment_Count_Data_All_coord, aes(x=Percent_Forest, y=Land_Surface_Temp, color="Forest"), size=2) +
  geom_point(data=Treatment_Count_Data_All_coord, aes(x=Percent_Developed, y=Land_Surface_Temp, color="Developed"), size=2) +
  xlim(0, 100) + 
  xlab("Land Use and Land Cover (%)") +
   ylab("Land Surface Temperature (°C)") +
    labs(color = "Land Use Land Cover") +
   ggtitle("Relationship of LULC, NDVI, and LST near K-12 Schools in Calfornia, USA") + 
  theme_bw() + theme(plot.title = element_text(hjust = 0.5))
ggsave(filename = 'LULC_NDVI_LST.png', plot = last_plot(), width = 9, height = 7, units = "in", dpi = 300)
```

# Heatmap of Occupant Health by Intervention and Utility Service Area
```{r}
library(dplyr)
library(readr)
Scenario_HeatMap_Data <- read_csv("Scenario_HeatMap_Data.csv")
Scenario_HeatMap_Data_2 <- Scenario_HeatMap_Data %>%
  mutate(Scenario = coalesce(Scenario_1_Type, Scenario_2_Type,	Scenario_3_Type))
# Load the required libraries 
install.packages("reshape2")
library(ggplot2)
library(reshape2)
data1 <- melt(Scenario_HeatMap_Data_2)
ggplot(data1, aes(x = Scenario, y = Utility, fill = value)) +
  geom_tile(color = "black") +
  geom_text(aes(label = value), color = "white", size = 4) +
  scale_fill_gradient(low = "blue", high = "red") +
  coord_fixed() + 
  labs(title = "Average Rate of Chronic Absenteeism by Intervention and Utility Service Area in K-12 Schools Across California",
       x = "Scenario",
       y = "Utility",
      fill = "(%)") + 
  theme_bw() + theme(plot.title = element_text(hjust = 0.5))  # Center ggplot title
ggsave(filename = './figures/', 'Scenarios_by_Utility.png', plot = last_plot(), width = 9, height = 7, units = "in", dpi = 300) 
```
