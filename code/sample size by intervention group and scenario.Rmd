...
#Scenario 3LHB: Lighting + HVAC + Building Envelope in K-12 Schools
Treatment_Count_S3_SubLHB <-Treatment_Count_subLHB %>% group_by(retrofit_and_ndvi) %>% 
  summarize(count_distinct = n_distinct(CDSCode))

#Scenario 3LHP: Lighting + HVAC + Pumps, Motors, and Drives in K-12 Schools
Treatment_Count_S3_SubLHP <-Treatment_Count_subLHP %>% group_by(retrofit_and_ndvi) %>% 
  summarize(count_distinct = n_distinct(CDSCode))

#Scenario 3HBP: HVAC + Building Envelope + Pumps, Motor, & Drives in K-12 Schools
Treatment_Count_S3_SubHBP <-Treatment_Count_subHBP %>% group_by(retrofit_and_ndvi) %>% 
  summarize(count_distinct = n_distinct(CDSCode))

#Scenario 3BLP: Building Envelope + Lighting + Pumps, Motors, & Drives in K-12 Schools
